package com.chinook.app.io.sqlite;
...
import com.chinook.app.io.Task_DatabaseIO;
...
//			return results;
		...
		case DatabaseConstants.QUERY_BUILDING_LIST:
			query_BuildingList();
			return this;
		
		case DatabaseConstants.QUERY_FLOOR_LIST:
			query_FloorList();
			return this;
		
		case DatabaseConstants.QUERY_TYPE_LIST:
			query_TypeList();
			return this;
		
		case DatabaseConstants.QUERY_NODES_BY_BUILDING_FLOOR_TYPE:
			query_NodesByFloorAndType();
			return this;
...
//		}
	...
	private boolean query_FloorList() {
		// open the SQLite database
		Log.i("SQLITE","begin query_FloorList");
		if (!openDatabaseConnection()) {return false;}
		...
		cursor = db.query(true, // distinct
				DatabaseConstants.TABLE_NAME, // table name
				new String[] { DatabaseConstants.KEY_FLOOR_LEVEL }, // table columns returned
				DatabaseConstants.KEY_BUILDING_ID + "='" + params[0] + "'", // where clause
				null, // selection args
				null, // groupBy
				null, // having
				DatabaseConstants.KEY_FLOOR_LEVEL, // orderBy
				null // limit
				);
		...
		if (cursor != null) {
			while (cursor.moveToNext()) {
				this.results.add(cursor.getString(cursor.getColumnIndex(DatabaseConstants.KEY_FLOOR_LEVEL)));
			}
		} else {
			this.results.add(DatabaseConstants.RESULT_FAILED);
		}
		//cursor.close();
		Log.i("SQLITE","query_FloorList- results.size: " + this.results.size());
		return true;
	}

	private boolean query_TypeList() {
		Log.i("SQLITE","begin query_TypeList");
		// open the SQLite database
		if (!openDatabaseConnection()) {return false;}
		...
		cursor = db.query(true, // distinct
				...
				null, // groupBy
				null, // having
				DatabaseConstants.KEY_NODE_TYPE, // orderBy
				null // limit
				);
		...
		if (this.cursor != null) {
			while (cursor.moveToNext()) {
				this.results.add(cursor.getString(cursor.getColumnIndex(DatabaseConstants.KEY_NODE_TYPE)));
			}
		} else {
			this.results.add(DatabaseConstants.RESULT_FAILED);
		}
		//cursor.close();			
		Log.i("SQLITE","query_TypeList- results.size: " + this.results.size());
		return true;
	}
	
	private boolean query_BuildingList() {
		Log.i("SQLITE","begin query_BuildingList");
		// open the SQLite database
		if (!openDatabaseConnection()) {return false;}
	...
	private boolean query_NodesByFloorAndType() {
		// open the SQLite database
		if (!openDatabaseConnection()) {return false;}
		...
		cursor = db.query(true, // distinct
				...
				null, // groupBy
				null, // having
				DatabaseConstants.KEY_NODE_LABEL, // orderBy
				null // limit
				);
