...
public class ProgressManager implements Runnable {
	...
	public ProgressManager(ProgressBar mProgressBar, Activity mActivity, int tasktype, int value, String message){
		...
		this.indeterminateBar = true;	// by default, we show indeterminate progress bar
	}
	
	// constructor for progress dialog
	public ProgressManager(ProgressDialog mProgressDialog, int tasktype, String message){
		this.mActivity = null;
	...
	private void showProgressBar() {
		if (indeterminateBar) {
			...
			// this statement has to be made before "setContentView(R.layout.xxxx)"
			...
			// we are not using horizontal bars in this app, so I wont bother coding this
			//if (titleBar) activity.requestWindowFeature(Window.FEATURE_PROGRESS);
