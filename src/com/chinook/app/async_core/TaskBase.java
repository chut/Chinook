package com.chinook.app.async_core;
...
public abstract class TaskBase<V, E1, E2> implements Callable<V>, OnCancelListener {
	...
	public final void addProgressDialog(Context context, String title, String message) {
	...
	public final void toggleProgress() {
		...
		if (mProgressDialog != null || bHasIndeterminateProgressBar || mProgressBar != null) {
			if (bToggleProgress) {
				...
				if(mProgressDialog != null) this.handlerUI.post(new ProgressManager(mProgressDialog, AsyncConstants.PROGRESS_CLOSE_DIALOG, null));
				if(bHasIndeterminateProgressBar || mProgressBar != null) {
					Log.i("TAG","toggleProgress - off");
					this.handlerUI.post(new ProgressManager(mProgressBar, mActivity, AsyncConstants.PROGRESS_CLOSE_BAR, 0, null));
				}
				...
				if(mProgressDialog != null) this.handlerUI.post(new ProgressManager(mProgressDialog, AsyncConstants.PROGRESS_SHOW_DIALOG, null));
				if(bHasIndeterminateProgressBar || mProgressBar != null) {
					Log.i("TAG","toggleProgress - on");
					this.handlerUI.post(new ProgressManager(mProgressBar, mActivity, AsyncConstants.PROGRESS_SHOW_BAR, 0, null));
				}
			}
		}
	}
